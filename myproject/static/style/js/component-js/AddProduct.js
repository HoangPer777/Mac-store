...
document.addEventListener('DOMContentLoaded', function () {
    // Tìm tất cả checkbox có liên quan đến trường "is_primary"
    const primaryCheckboxes = document.querySelectorAll('input[type=checkbox][name$=is_primary]');

    primaryCheckboxes.forEach((checkbox) => {
        checkbox.addEventListener('change', function () {
            if (this.checked) {
                // Khi checkbox được tick, bỏ chọn tất cả checkbox khác
                primaryCheckboxes.forEach((box) => {
                    if (box !== this) {
                        box.checked = false;
                    }
                });
